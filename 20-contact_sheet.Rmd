...
```{r}
broken_mri <- images_info("../80_images") %>%
...
im <- get_img_mask(broken_mri$file_path[[1]])
plotBlendedImages(im$image, mask = im$mask)
```


```{r}
slice <- ANTsRCore::extractSlice(
  broken_mri$file_path, 
  slice = 1, direction = 3
)
ch1 = ANTsRCore::splitChannels(slice)[[1]]

invisible(plot(ch1, doCropping = FALSE))
```
