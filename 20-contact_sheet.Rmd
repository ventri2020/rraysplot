...
```{r test-mask-1, eval=FALSE, fig.asp=0.24, fig.show="hold", include=FALSE, out.width="100%"}
patient_contact_sheet(broken_test_masks[1], set = "test")
```

```{r}
broken_test_images <- images_info("../80_images") %>%
  dplyr::filter(patient %in% broken_test_masks & kind == "MRI")
broken_test_images
```

```{r}
broken_train_images <- images_info("../80_images") %>%
  dplyr::filter(patient %in% broken_train_masks & kind == "MRI")
broken_train_images
```

```{r}
get_img_mask <- function(path) {
...
```{r broken-test}
im <- get_img_mask(broken_test_images$file_path[[1]])
plotBlendedImages(im$image, mask = im$mask)
```

```{r broken-train, echo=FALSE, fig.asp=0.24, fig.show="hold", out.width="100%"}
...
```{r blended-train, echo=FALSE, fig.asp=0.24, fig.show="hold", out.width="100%"}
im1 <- get_img_mask(broken_train_images$file_path[[1]])
p1 <- plotBlendedImages(im1$image, mask = im1$mask)
im2 <- get_img_mask(broken_train_images$file_path[[2]])
p2 <- plotBlendedImages(im2$image, mask = im2$mask)
im3 <- get_img_mask(broken_train_images$file_path[[3]])
